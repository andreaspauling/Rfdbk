...
fdbk_dt_add_obs_ini<-function (DT, fnamepast, cond = cond) 
{
    ...
    if (length(fillFiles) > 0) {
		...
		DTFILL = try(fdbk_dt_multi_large(fillFiles[i],condition = cond, vars = vars, cores = 1),T)
		if (class(DTFILL)[1]=="try-error") next()
		if (dim(DTFILL)[1]==0) next()
		DTFILL = unique(DTFILL)
...
#'
#' Helps to filter rows from data.tables with condition for the same column.
#' A typical application would be to filter wind direction observations whenever wind speed is <3m/s 
#' In this case one would have to delete all varno==111 for all matching varn===112 that have obs less than 3m/s.
#' This is complex, as observations of varno 111 and 112 are in different rows.
#' Even more complicated fitering task, e.g. filtering a variable on more than one other variable, might require to call this
#' function sepatately for each task.
#'
#' @param DT the data.table
#' @param condition logical condition as character string
#' @param on logical condition(s) as character string
#' @param by character vector of all column names that sould be used for the grouping
#' 
#' @return the fitered data.table (column and row order is not preserved!)
#'
#' @author Felix <felix.fundel@@dwd.de>
#'
#'
...
fdbk_dt_conditional <-function(DT,condition,on,by){
	print(paste0("I am removing all ",condition," whenever no! matching observation with ",on," can be found"))
	expr = paste(".rbind.data.table(DT[!",condition,"],merge(DT[",condition,"],DT[",on,",",paste0("c('",paste(by,collapse="','"),"')"),",with=F],by=",paste0("c('",paste(by,collapse="','"),"')"),"))")
	#print(expr)
